...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const comments = await commentDB.getComments(client, postId);
    const totalCount = await commentDB.getCount(client, postId);

    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ONE_COMMENT_SUCCESS, comments, totalCount));
