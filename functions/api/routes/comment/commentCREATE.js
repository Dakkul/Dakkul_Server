...
module.exports = async (req, res) => {
  ...
  if (!postId || !content) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
  ...
  try {
    ...
    const comment = await commentDB.createComment(client, content, postId);
    
    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.ADD_ONE_COMMENT_SUCCESS, comment));
